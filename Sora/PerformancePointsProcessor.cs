...
namespace Sora
{
    ...
    public class ProcessorScoreParser : LegacyScoreParser
    {
        ...
        {
            IConvertibleReplayFrame convertible = ruleset.CreateConvertibleReplayFrame();
            if (convertible == null)
            ...
            ReplayFrame frame = (ReplayFrame) convertible;
        ...
        public Score Parse(Database.Models.Scores dbScore)
        {
            using FileStream rawReplay = File.OpenRead("data/replays/" + dbScore.ReplayMd5);
            ...
            long compressedSize = rawReplay.Length - rawReplay.Position;
            ...
            using (LzmaStream lzma = new LzmaStream(properties, rawReplay, compressedSize, outSize))
            using (StreamReader reader = new StreamReader(lzma))
    ...
    {
        public double Compute(Database.Models.Scores dbscore)
        {
            ProcessorWorkingBeatmap workingBeatmap = new ProcessorWorkingBeatmap("data/beatmaps/" + dbscore.FileMd5);
            ProcessorScoreParser psp = new ProcessorScoreParser(workingBeatmap);
            Score score = psp.Parse(dbscore);
            
            Dictionary<string, double> categoryAttribs = new Dictionary<string, double>();
            double pp = score.ScoreInfo.Ruleset
                             .CreateInstance()
